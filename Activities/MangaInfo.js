            ...
            await Auth.currentAuthenticatedUser()
                .then(async (user) => {
                    hash(user.username).then(async (client) => {
                        ...
                        const list_fav = userById.data.UserByClienID.items[0].list_favoris_manga;
                        if (list_fav == null) {
                            setClient_favoris([])
                        }
                        else {
                        ...
                        if (current_chap.data.ReadingByClienID.items.length === 0) {
                            ...
                            try {
                                API.graphql(graphqlOperation(createReading, {
                                    input:
                                    {
                                        clienID: client,
                ...
                hash(user.username).then(async (clientID) => {
                    try {
                        ...
                        if (is_in_lib) {
                            ...
                            await API.graphql(graphqlOperation(updateUser, { input: { id: userByIDData.id, list_favoris_manga: client_favoris } }))
