...
import { listMangas, listAnimes } from '../graphql/queries';

const HeaderSearch = ({ setStateData, type, toogle }) => {
    ...
    async function searchMangas(txt) {
        ...
        try {
            ...
            setStateData(mangaData.data.listMangas.items.sort(
    ...
    async function searchAnime(txt) {

        const formalized_text = txt.toUpperCase().split(" ").join("").replace(/[^a-zA-Z0-9]/g, '')

        try {
            const animeData = await API.graphql(graphqlOperation(listAnimes, { filter: { title_search: { contains: formalized_text } } }))
            setStateData(animeData.data.listAnimes.items.sort(
                    ...
                    onChangeText={(newVal) => {
                        ...
                        switch (type) {
                            ...
                            case "anime":
                                ...
                                break;
                            default:
                                console.log("error type for research")
                        }
